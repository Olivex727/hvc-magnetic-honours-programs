...
"""
~MWE script for demonstrating read-in of ASKAP FDFs, calculate RMs.
"""
...
basedir = '/path/to/parent/datadir/'
relatve_dir_fdf_amp_spectra = './amplitude/'
relatve_dir_fdf_phase_spectra = './phase/'
relative_dir_iquv_noise_spectra = './noise/'
...
os.chdir(basedir + relatve_dir_fdf_amp_spectra)
...
for fidx,file in enumerate(files):
	...
	result_FDF_amp = load_and_parse_fits(file)
	result_FDF_phase = load_and_parse_fits(basedir + relatve_dir_fdf_phase_spectra + file.replace('amp','phase'))
	...
	BAN_recorder = None	
	for stokes in ['Q','U']:
		stokes_noise_spectrum_file = basedir + relative_dir_iquv_noise_spectra + file.replace('pol_FDF_amp','pol_noise_%s'%stokes)
	...
	RM_accumulator.append(phi_peak_fitted_radmm)
	RMerr_accumulator.append(dphi_peak_fitted_radmm)
	polSN_accumulator.append(SN_PI_fitted)
...
RM_accumulator_arr = np.array(RM_accumulator)
RMerr_accumulator_arr = np.array(RMerr_accumulator)
polSN_accumulator_arr = np.array(polSN_accumulator)
...
table_data = {
	...
	'rm': radec_obj_accumulator_arr,
	'rm_uncert': radec_accumulator_arr,
	'SN_PI_fitted': polSN_accumulator_arr,
}
...
t['rm'].description = 'The Faraday Rotation Measure in rad/m/m'
t['rm_uncert'].description = 'The uncertainty in Faraday Rotation Measure in rad/m/m'
t['SN_PI_fitted'].description = 'The signal:noise ratio in polarisation from RM synthesis'
